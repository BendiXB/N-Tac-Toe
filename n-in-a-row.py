...
class player:   # class containing the players functions
    ...
    def place(self):    # places the players symbol
        try:
            x = int(input('Enter '+formatting.blue+'x position'+formatting.end+' for '+self.name+':')) - 1  # input x-coordinate with index starting at 1
            y = int(input('Enter '+formatting.red+'y position'+formatting.end+' for '+self.name+':')) - 1   # input y-coordinate with index starting at 1
    ...
    def checkforwin(self):  # method to check for a win
        ...
        inrows = []     # list of all combinations of symbols in a rows horizontally, verticaly and diagonal (inrow) to check for a win
            ...
            for positionwidth in range(game.width): # iterate through rows
                ...
                for step in range(game.inarow): # stepping inarow times
                    ...
                    try:
                        ...
                        pass            # do nothing ang go on because these cases dont met winning conditions
                    try:
                        diagonalrightlist.append(game.map[positionheight + step, positionwidth + step]) # getting next step for inrow diagonaly left
                    except IndexError:  # excepting the case that a inrow goes out of board
                        pass            # do nothing ang go on because these cases dont met winning conditions
                    try:
                        diagonalleftlist.append(game.map[positionheight + step, positionwidth - step])  # getting next step for inrow diagonaly right
                    except IndexError:  # excepting the case that a inrow goes out of board
...
class game:     # class containing all game assets and mainloop
    ...
    def printmap(self):     # function to print the map
        ...
        print('    ┃ '+formatting.blue+(formatting.end+' │ '+formatting.blue).join(map(str, list(range(1, self.width+1))))+formatting.end+' │')  # print top of board by printing column numbers seperated by |
        print('┍━━━╋'+(self.width - 1)*'━━━┿'+'━━━┥')           # print seperator betwenen top line and map
        rownr = 1  # number of current row
        for row in self.map.tolist():    # iterating through rows of map except last
            print('│ '+formatting.red+str(rownr)+formatting.end+' ┃ '+' │ '.join(list(map(str, row)))+' │')  # printing all fields in a row separated by a │ with line number
            if rownr == len(self.map.tolist()):                     # if last row:
                print('└───┸' + (self.width - 1) * '───┴' + '───┘') # print last linen
                break   # end loop before a standart line can be placed under last line
